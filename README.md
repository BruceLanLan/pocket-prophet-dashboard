...
| 🔯 **奇门遁甲** | 按当前时间起盘（节气定局/阴阳遁/值符值使/三盘，排盘引擎复用自作者另一个命理项目），3×3 九宫格显示门/星/干支，粗框标出值符宫 |

摇卦和奇门遁甲是"点一下就推、推了就看"的独立按钮，跟起卦/起盘这个动作本身的仪式感一致；天气/行情/新闻/用量四页是配置驱动的内容页，支持先"预览"（只调用转换接口生成预览图，**不会**真的推给设备）再决定要不要"推送"。
...
- **目前只在 macOS 上验证过。** `renderer/base.py` 里的中文字体路径硬编码成了 macOS 系统自带的 `/System/Library/Fonts/STHeiti Medium.ttc`；Linux/Windows 上会因为找不到这个文件直接报错。要跨平台跑，需要自己改成对应系统上的中文字体路径。
- **外部数据源都是免费公开接口，没有 SLA。** wttr.in、Yahoo Finance、RSS 源、今日头条热榜——这些都不是官方合作接口，偶尔超时/限流是正常现象，不代表代码坏了；`docs/PLAN.md`/`docs/PLAN-v2.md` 里记录过好几次这类真实的网络抖动。
- **这是个人用的本地工具，不是要对外提供服务的产品。** 跑的是 Flask 自带的开发服务器（`app.run` 默认配置），单进程、没做并发/鉴权加固，设计前提是"只有你自己在局域网里访问"。不要把它暴露到公网。
- **设备连通性由设备本身的模式决定，不是这个项目能控制的。** 见上面「快速开始」——这不是 bug，是这块墨水屏的固件行为。
